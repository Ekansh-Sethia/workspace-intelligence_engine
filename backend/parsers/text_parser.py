...
class TextParser(BaseParser):
    ...
    def parse(self, file_content: bytes, filename: str, source_path: Optional[str] = None) -> Document:
        ...
        """
        text = self._read_with_fallback(file_content, filename)
    ...
    def _read_with_fallback(self, file_content: bytes, filename: str) -> str:
        """Try each encoding in turn; raise on total failure."""
        for enc in self._ENCODINGS:
            try:
                return file_content.decode(encoding=enc)
